...
const CodeDetailsModel = ({ show, setShow }) => {
    const { selectExecution } = ExecutionState();
    return (
        <Fragment >
            <div className="fixed h-screen w-full z-50 left-0 top-0 transition duration-300 ease-in-out" style={{ display: show ? 'block' : 'none' }}>
                ...
                <div className="w-[95%] lg:w-3/4 h-fit max-h-5/6 my-6 mx-auto bg-white p-3 space-y-1 shadow-md rounded-lg border-4 border-spacing-4 border-gray-400">
                    ...
                    <p className='w-full text-sm text-gray-600'>By
                        <span className='font-medium text-black ms-1'>{selectExecution?.submitBy ? selectExecution.submitBy.username : "unknow"}</span>,
                    ...
                    <div className='w-full h-fit max-h-[88%] overflow-y-auto space-y-1'>
                        <div className='w-full h-fit px-3 py-4 bg-gray-200 text-sm'>
                            {selectExecution?.code}
                        </div>
                        <hr />
                        <p className='text-xs'>Output</p>
                        <div className='w-full h-fit px-3 py-4 bg-gray-200 text-sm'>
                             Unfontunately I can't use any compiler here. So, can't show this code result.
                        </div>
                    </div>
                </div>
            </div>
